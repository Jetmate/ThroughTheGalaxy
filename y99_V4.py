...
class Game:
    ...
    do = False
    game = 'go'
    def __init__(self,canvas, width, height, colors, number, x_size, y_size, ship, asteroids):
        self.colors = colors
    ...
    def draw(self):
        ...
        self.ship = self.canvas.create_image(center[0], center[1], anchor = NW, image = self.image)
    ...
    def create_asteroid(self, asteroid_x):
        ...
        asteroid = self.canvas.create_image(asteroid_coordinates)
    ...
    def move_asteroid(self):
        for asteroid in self.asteroids:
            ...
            if self.check_collision(self.asteroids[asteroid][1], self.coordinates):
                self.game = 'lose1'
                self.canvas.delete("all")
                self.canvas.config(background = 'grey5')
                sleep(.3)
...
canvas.pack()
image = PhotoImage(file = "C:\\Python34\\Untitled.gif")
#beggining()

game = Game(canvas, 1000, 500, colors, 11, 20, 9, image)
game.prep()
while True:
    if game.game == 'go':
        if game.do:
